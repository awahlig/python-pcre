...
#
# Modifications have been commented with "PCRE:".
# Imports pcre instead of re.
# Internal _sre tests have been commented out.
# Not supported features have been commented out.
# Regular expressions have been changed to PCRE format.
# --------------------------------------------------------------------------
...
# PCRE: import pcre instead of re
#import re
...
class ReTests(unittest.TestCase):
    ...
    def test_repeat_minmax(self):
        ...
        self.assertNotEqual(re.match("^x{3}?$", "xxx"), None)
        self.assertNotEqual(re.match("^x{1,3}?$", "xxx"), None)
        # PCRE: need to make ungreedy for this case to work in PCRE
        #self.assertNotEqual(re.match("^x{1,4}?$", "xxx"), None)
        self.assertNotEqual(re.match("(?U)^x{1,4}?$", "xxx"), None)
        self.assertNotEqual(re.match("^x{3,4}?$", "xxx"), None)
    ...
    def test_special_escapes(self):
        ...
        # PCRE: python-pcre doesn't support LOCALE flag
        #self.assertEqual(re.search(r"\b(b.)\b",
    ...
    def test_string_boundaries(self):
        ...
        # non-boundaries.
        # PCRE: PCRE needs the NOTEMPTY_ATSTART flag to pass
        #self.assertEqual(re.search(r"\B", ""), None)
        self.assertEqual(re.compile(r"\B").search("", flags=re.NOTEMPTY_ATSTART), None)
    ...
    def test_big_codesize(self):
        # Issue #1160
        # PCRE: Size needs to be reduced to pass.
        #r = re.compile('|'.join(('%d'%x for x in range(10000))))
        r = re.compile('|'.join(('%d'%x for x in range(6000))))  # PCRE
    ...
    def test_getlower(self):
        # PCRE: we're not testing _sre here
        #import _sre
    ...
    def test_sre_character_literals(self):
        ...
        # PCRE: PCRE doesn't see this as an error
    ...
    def test_bug_527371(self):
        ...
        # PCRE: PCRE's lastgroup/lastindex is based on opening parentheses
        #self.assertEqual(re.match("(?P<a>a(b))", "ab").lastgroup, 'a')
        #self.assertEqual(re.match("((a))", "a").lastindex, 1)
        self.assertEqual(re.match("(?P<a>a(b))", "ab").lastgroup, None)
        self.assertEqual(re.match("((a))", "a").lastindex, 2)
    ...
    # PCRE: python-pcre doesn't support scanner APIs
    def __test_scanner(self):
    ...
    def test_inline_flags(self):
        ...
        # PCRE: UNICODE mode can be enabled using (*UCP) instead of (?u)
        #p = re.compile('(?iu)' + upper_char)
        p = re.compile('(*UCP)(?i)' + upper_char)
        q = p.match(lower_char)
        self.assertNotEqual(q, None)

        # PCRE: UNICODE mode can be enabled using (*UCP) instead of (?u)
        #p = re.compile('(?iu)' + lower_char)
        p = re.compile('(*UCP)(?i)' + lower_char)
        q = p.match(upper_char)
        self.assertNotEqual(q, None)
    ...
    def test_repeat_minmax_overflow(self):
        ...
        # PCRE: PCRE requires "{0,x}" instead of "{,x}"
        # PCRE: PCRE doesn't support repetition over 64K
        # PCRE: 2**128 repetition raises NO error -- PCRE bug?

        string = "x" * 100000
        self.assertEqual(re.match(r".{65535}", string).span(), (0, 65535))
        self.assertEqual(re.match(r".{0,65535}", string).span(), (0, 65535))
        self.assertEqual(re.match(r".{65534,}?", string).span(), (0, 65534))
        self.assertEqual(re.match(r".{65535}", string).span(), (0, 65535))
        self.assertEqual(re.match(r".{0,65535}", string).span(), (0, 65535))
        self.assertEqual(re.match(r".{65535,}?", string).span(), (0, 65535))
        self.assertRaises(OverflowError, re.compile, r".{%d}" % 65536)
        self.assertRaises(OverflowError, re.compile, r".{0,%d}" % 65536)
        self.assertRaises(OverflowError, re.compile, r".{%d,}?" % 65536)
        self.assertRaises(OverflowError, re.compile, r".{%d,%d}" % (65537, 65536))
        return
    ...
    def test_repeat_minmax_overflow_maxrepeat(self):
        try:
            # PCRE: this is defined in pcre module (and is always 64K)
        ...
        self.assertEqual(re.match(r".{0,%d}" % (MAXREPEAT - 1), string).span(),
                         (0, 10000))
        self.assertIsNone(re.match(r".{%d,}?" % (MAXREPEAT - 1), string))
        self.assertRaises(OverflowError, re.compile, r".{%d}" % MAXREPEAT)
        self.assertRaises(OverflowError, re.compile, r".{0,%d}" % MAXREPEAT)
        self.assertRaises(OverflowError, re.compile, r".{%d,}?" % MAXREPEAT)
        return
    ...
    # PCRE: PCRE's errors don't contain group names
    def __test_backref_group_name_in_exception(self):
        # Issue 17341: Poor error message when compiling invalid regex
        with self.assertRaisesRegexp(sre_constants.error, '<foo>'):
            re.compile('(?P=<foo>)')

    # PCRE: PCRE's errors don't contain group names
    def __test_group_name_in_exception(self):
        # Issue 17341: Poor error message when compiling invalid regex
        with self.assertRaisesRegexp(sre_constants.error, '\?foo'):
            re.compile('(?P<?foo>)')
    ...
    # PCRE: python-pcre doesn't support DEBUG flag
    def __test_debug_flag(self):
...
def run_re_tests():
    # PCRE: use pcre_tests instead of re_tests
    #from test.re_tests import tests, SUCCEED, FAIL, SYNTAX_ERROR
    from test.pcre_tests import tests, SUCCEED, FAIL, SYNTAX_ERROR
    if verbose:
        #print 'Running re_tests test suite'
        print 'Running pcre_tests test suite'
    else:
